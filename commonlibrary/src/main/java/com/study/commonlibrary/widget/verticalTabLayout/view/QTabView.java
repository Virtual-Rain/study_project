...
public class QTabView extends TabView {
    ...
    public boolean isNeayByChecked;
    ...
    private void initView() {
        setMinimumHeight(dp2px(mContext, 25));
        if (mTitle == null) {
    ...
    public void setInvisibleIcon() {
        refreshIconDrable(mTabIcon.getSelectedIcon());
    }

    public void setVisibleIcon() {
        refreshIconDrable(mTabIcon.getNormalIcon());
    }

    public void initIconView() {
        int iconResid = mChecked ? mTabIcon.getSelectedIcon() : mTabIcon.getNormalIcon();
        refreshIconDrable(iconResid);
    }

    public void refreshIconDrable(int iconResid) {
        Drawable drawable = null;
        if (iconResid != 0) {
            drawable = ContextCompat.getDrawable(mContext, iconResid);
    ...
    private void setLayoutParams() {
        ...
        params.gravity = mTabTitle.getTitleGravity();
    ...
    /**
     * @param resId The Drawable res to use as the ybcp_background, if less than 0 will to remove the
     *              ybcp_background
     */
    @Override
    ...
    public void setBackgroundNormal(boolean isNormal) {
        setBackground(isNormal ? mBackgroundResId : -1);
    }
}